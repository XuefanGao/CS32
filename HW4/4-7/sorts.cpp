...
void sortUsingPtrs(vector<Store>& stores, bool comp(const Store*, const Store*))
{
    ...
    for (int unsorted = 1; unsorted < n; unsorted++)
    {
        Store *nextItemPtr = storePtrs[unsorted];
        int loc = unsorted;
        while (loc > 0 && comp(nextItemPtr, storePtrs[loc - 1]))
        {
            storePtrs[loc] = storePtrs[loc - 1];
            loc--;
        }
        storePtrs[loc] = nextItemPtr;
    }
